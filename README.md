...
将 `security-scanner/` 目录挂载到 AI 编码工具的 skill 搜索路径后，在目标项目中发起扫描请求。本项目没有提供独立命令行入口；扫描执行由 AI agent 根据 `SKILL.md`、`orchestration/`、`scanners/`、`references/` 和 `templates/` 中的指令完成。

也可以复制整个 `security-scanner/` 目录到对应 skill 目录；下面示例使用软链接，便于后续同步更新。
...
`redline-full` 是默认完整扫描 profile；需要快速预检时可替换为 `redline-p0`，只看二进制或交付包时可替换为 `redline-binary`。替换 profile 只影响 Phase 1 扫描调度，不改变 Phase 3 固定报告产物。
...
ln -s /abs/path/to/security-scanner .claude/skills/security-scanner
claude
```
...
ln -s /abs/path/to/security-scanner .agents/skills/security-scanner
...
```bash
codex --cd /path/to/project '$security-scanner 请使用 security-scanner 对当前目录执行 redline-full 安全合规扫描。请严格按 Phase -1 环境预检、Phase -0 输入物化、Phase 0 发现、Phase 1 扫描、Phase 2 裁决、Phase 3 报告生成执行。无论 scan_profile 是否跳过部分维度，最终必须在 security-reports/ 生成综合 Markdown、JSON、component-info summary JSON 和 13 份维度独立详细 Markdown 报告；跳过、降级或失败维度也要生成占位报告并写明原因。'
```
...
ln -s /abs/path/to/security-scanner .opencode/skills/security-scanner
opencode
```
...
```text
使用 security-scanner skill 对当前目录执行 redline-full 安全合规扫描。
请严格按 Phase -1 环境预检、Phase -0 输入物化、Phase 0 发现、Phase 1 扫描、Phase 2 裁决、Phase 3 报告生成执行。
无论 scan_profile 是否跳过部分维度，最终必须在 security-reports/ 生成综合 Markdown、JSON、component-info summary JSON 和 13 份维度独立详细 Markdown 报告；跳过、降级或失败维度也要生成占位报告并写明原因。
```
...
| 检查点 | 阶段 | 重点 |
|--------|------|------|
| A-0 | Materialize | 输入类型识别、SRPM `%prep` 物化、binary RPM 文件系统视图、source_roots / binary_roots 完整性 |
...
| A3c | Report Manifest | 综合 Markdown、JSON、component-info summary JSON 和 13 份维度独立详细报告的实际产物清单完整性 |
...
可以让 Phase 1 扫描范围变窄。用户可以在自然语言中指定维度，例如“只检查口令硬编码和公网地址”，Agent 可只加载相关 scanner 和必要 reference 文件，并将其他维度记录为跳过或不适用。

但 Phase 3 仍固定生成 13 份维度独立详细报告。未扫描、跳过、降级或失败的维度必须生成占位报告，并在报告中写明原因。
