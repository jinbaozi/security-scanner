...
def summarize(scan_plan: dict[str, Any], sample_size: int) -> dict[str, Any]:
    ...
    all_files = scan_plan.get("all_files", [])
    source_items: list[Any] = []
    if isinstance(shards, list):
        for shard in shards:
            if isinstance(shard, dict) and isinstance(shard.get("files"), list):
                source_items.extend(shard["files"][:sample_size])
                if len(source_items) >= sample_size:
                    break

    samples = {
        "elf": _samples(scan_plan.get("elf_files"), sample_size),
        "source": _samples(source_items or all_files, sample_size),
        "config": _samples(scan_plan.get("config_files"), sample_size),
        "excluded": _samples(scan_plan.get("excluded"), sample_size),
    }
    ...
    return {
        ...
        "elf_count": _count(scan_plan.get("elf_files")),
        "config_count": _count(scan_plan.get("config_files")),
        "dependency_count": _count(scan_plan.get("dependency_files")),
        "excluded_count": _count(scan_plan.get("excluded")),
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            f"scan-plan-summary status=failed reason={type(exc).__name__}",
            file=sys.stderr,
        )
        return 4
