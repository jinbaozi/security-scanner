...
| `check_item` | `insecure_symmetric`、`insecure_asymmetric`、`insecure_hash`、`credential_storage`、`pseudo_encryption`、`insecure_random`、`library_vuln`、`legacy_protocol_crypto`、`custom_algorithm` |
...
3. **Hash 算法**：识别 MD5/SHA-1/MD2/MD4/HMAC-*-96 是否在密码、签名、证书、认证码等敏感场景使用。
4. **自定义算法**：识别自写 XOR、字符串反转、Caesar 移位等伪加密。
5. **推荐国密算法**：识别 SM2/SM3/SM4 时仅输出 `INFO` 或写入组件档案，不作为 FAIL。

### Step 3b: 凭据存储检测（6.1.1）

识别口令、工作密钥或认证材料以明文、弱哈希或可逆伪加密形式存储的场景，输出 `check_item=credential_storage`：

```bash
# 明文口令/密钥存储
grep -rnE "(password|passwd|pwd|secret|token|api_key|access_key|private_key)\s*[:=]\s*[\"'][^\"']+[\"']" {source_and_config_files}

# MD5(用户名 || 口令)、MD5(password)、SHA1(password)
grep -rnE "(md5|MD5|sha1|SHA1)\s*\([^)]*(user(name)?|account|login)[^)]*(password|passwd|pwd)" {source_files}
grep -rnE "(md5|MD5|sha1|SHA1)\s*\([^)]*(password|passwd|pwd)" {source_files}
```

判定规则：

- 明文凭据落盘或配置默认值：`status=FAIL`、`severity=high`，`redline_clause=6.1.1`。
- `MD5(username || password)`、无盐 MD5/SHA1 口令摘要：`status=FAIL`、`severity=high`，建议改用 PBKDF2/bcrypt/scrypt/Argon2 并加盐。
- 若只是变量名命中但无法证明存储或认证用途：`status=WARN`、`verdict=needs_human`。
...
# Session 标识/认证 token
grep -rnE "Math\.random[\s\S]{0,120}(session|sid|token|csrf|nonce)" {js_files}
grep -rnE "(session|sid|token|csrf|nonce)[\s\S]{0,120}Math\.random" {js_files}
```

伪 RNG 必须上下文包含 key/iv/salt/token/nonce/secret/password 才告警（severity=critical）。若 `Math.random`、`java.util.Random`、`random.random` 等用于 session id、CSRF token、认证 nonce 或登录态 token，输出 `status=WARN`、`severity=medium`、`redline_clause=3.1.3`，建议改用系统 CSPRNG。
...
若 Dependency Scanner 已提供 SBOM 或漏洞 finding，优先消费其 `artifacts.sbom` 与 `known_vulnerability` 结论补充 evidence。Crypto Scanner 不产出 `MISSING_LOCK_FILE`；缺少 lock 文件由 Dependency Scanner 主责报告。
...
- 签名、证书、完整性保护、认证 token 派生：`FAIL`。
- HMAC-SHA1 兼容协议：默认 `WARN-PASS`，需在 `verdict_reasoning` 中说明协议上下文和人工确认点。
- Git hash、文件内容指纹、缓存 key、去重：不作为问题，必要时标记 `rejected` 并说明非安全用途。
...
- `WARN`：可能命中红线但需要人工确认，或命中库版本知识库但需升级；命中 `RL-120` ~ `RL-125` 时必须为 `WARN` + `needs_human`。
- `PASS`：未发现任何问题。
...
| `credential_storage` | high | 明文凭据、弱哈希口令、MD5(用户名\|\|口令) |
