...
| `rl_ids` | 命中的 RL 规则 ID 列表；仅端口信息记录可为空 |
...
命中 `legacy_protocol` 规则后，关联 `../../references/red-line-rules.md` 中对应的红线（RL-100 ~ RL-119），并写入 `redline_clause` 与 `rl_ids`。
...
对每个特权端口或高危服务（21/23/69/110/143/161/3306/5432/6379/27017 等）输出 `check_item=port_privileged` 或 `insecure_service`、`severity=high/critical`、`status=FAIL` 的 finding，并关联对应红线（23 → RL-104，69 → RL-106，161 → RL-107/RL-108，21 → RL-110）。
