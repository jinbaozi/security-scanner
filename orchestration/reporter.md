...
> 本文件指导 Reporter Agent 汇总裁决后的 findings，生成终端摘要、JSON 结构化数据、综合 Markdown 报告和按 profile 启用的维度专项报告。
...
- 报告模板：`templates/report-manifest.yaml` 声明的综合模板和当前 profile 对应专项模板。
- 执行维度清单：`executed_dimensions`，包含已执行、条件跳过或降级但需要报告占位的维度。
...
- 基本信息和扫描统计。
- 严重度汇总和各维度发现统计。
- 当前 profile 已执行维度的详细发现与对应专项报告索引。
- `executed_dimensions` 对应的维度状态摘要和独立详细报告索引。
...
Reporter 只为 `executed_dimensions` 中实际执行、条件跳过或降级的维度生成专项报告。专项报告只展示对应维度的数据，但必须继承 JSON 中的裁决结果、严重度和审计状态。综合报告必须提供这些独立详细报告的索引。

### 5. 报告完整性检查

所有预期输出文件生成后，必须执行完整性检查：

1. 根据 `templates/report-manifest.yaml` 和当前 `scan_profile`，列出所有预期输出文件：
   ...
   - 维度专项报告：`security-reports/report-{dim_display_name}-{component_name}-{date}.md`（仅当前 profile 实际执行或降级的维度）
2. 逐一检查各文件是否存在。
...
- `executed_dimensions` 中每个维度必须在 `templates/report-manifest.yaml` 中存在模板和输出路径。
- 每个已执行或降级维度必须生成对应独立详细报告；条件跳过维度必须在综合报告维度状态摘要中说明原因。
- 综合报告必须包含 `dimension_report_index` 和 `dimension_status_summary`，并与实际输出文件一致。
- 缺少任一独立详细报告、索引缺失或 `executed_dimensions` 与产物不一致均为 A3c FAIL。
...
- `executed_dimensions` 必须与专项报告产物、综合报告索引和降级说明一致。
- `templates/report-manifest.yaml` 是维度模板和输出命名的唯一来源。
- 独立详细报告缺失、维度状态无说明或索引指向不存在文件均为 FAIL。
