...
- 报告模板：`templates/` 目录下的综合模板和当前 profile 对应专项模板。
...
- `templates/report-依赖与漏洞.md` -> `report-依赖与漏洞-{component_name}-{date}.md`
- `templates/report-安全编码.md` -> `report-安全编码-{component_name}-{date}.md`
- `templates/report-完整性.md` -> `report-完整性-{component_name}-{date}.md`
- `templates/report-内容合规.md` -> `report-内容合规-{component_name}-{date}.md`

Reporter 只为当前 `scan_profile` 实际执行或降级的维度生成专项报告。专项报告只展示对应维度的数据，但必须继承 JSON 中的裁决结果、严重度和审计状态。
