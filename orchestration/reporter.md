...
- 输入物化元数据：Scan Plan 中的 `materialization` 对象，包括 `input_kind`、`source_roots`、`binary_roots`、`srpm_spec_files`、`applied_patches`、`builddep_attempted`、`builddep_status`、`status`、`errors`、`audit_log`。
...
- `materialization`: 输入物化摘要，原样保留 `materialization.status`、`input_kind`、SRPM spec、Patch、prepped source root、binary root、builddep_status、错误和降级原因。
...
- 基本信息和扫描统计。
- “输入物化摘要”：列出 SRPM、spec、Source、Patch、builddep_status、prepped source root、binary RPM、binary root、失败/降级原因。
- 严重度汇总和各维度发现统计。
